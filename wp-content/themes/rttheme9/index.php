...
if (get_option('rttheme_homepage_style')){
	$which_theme=get_option('rttheme_homepage_style');
}else{
	$which_theme="1";
}
?>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery("#menu-item-117").addClass("current_page_item");
    });
</script>
...
<div class="content productlist">
		...
		if(is_category()){//category
			$query_string='cat='.get_query_var('cat').'&paged='.$paged.'&posts_per_page='.get_option("rttheme_product_list_pager").'&post_status=publish&orderby=date&order=ASC';
		}elseif(get_option('rttheme_portf_start_cat')){//start page with start category
			$query_string='cat='.get_option('rttheme_product_start_cat').'&paged='.$paged.'&posts_per_page='.get_option("rttheme_product_list_pager").'&post_status=publish&orderby=date&order=ASC';
		}else{//start page with all categories

			$query_string='cat='.get_option('rttheme_product_list_ex_cat[]').'&paged='.$paged.'&posts_per_page='.get_option("rttheme_product_list_pager").'&post_status=publish&orderby=date&order=ASC';
		}
						...
						<div class="imgarea product_image preload">
							    ...
							    <?php if(get_post_meta($post->ID, 'rt_product_image_url', true)):?>
									     <a href="<?php echo get_permalink() ?>" title="<?php the_title(); ?>">
										    <img src="<?php bloginfo('template_directory'); ?>/timthumb.php?src=<?php echo get_post_meta($post->ID, 'rt_product_image_url', true)?>&amp;w=160&amp;h=168&amp;zc=1" alt="<?php the_title(); ?>" class="image product_image preload" />
									     </a>
							    <?php endif;?>
